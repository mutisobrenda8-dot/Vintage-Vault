// /vintage-vault/week6b//js/main.js
...
document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        fetch('/vintage-vault/week6b/cart_action.php', {
            method:  'POST',
        ...
        .then(data => {
            if (data.success) {
                ...
                setTimeout(() => {
                    window.location.href = '/vintage-vault/week6b/login.php';
                }, 1500);
                button.textContent = 'Add to Cart';
                button.disabled    = false;
            } else {
...
if (searchInput && suggestionsBox) {
    ...
    searchInput.addEventListener('input', function () {
        ...
        timer = setTimeout(() => {
            fetch(`/vintage-vault/week6b/search_suggestions.php?q=${encodeURIComponent(q)}`)
            .then(r => r.json())
            .then(results => {
                if (!results.length) {
                    suggestionsBox.style.display = 'none';
                ...
                suggestionsBox.innerHTML = results.map(item => `
                    <div onclick="window.location='/vintage-vault/week6b/product.php?id=${item.id}'"
    ...
    searchInput.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
            window.location.href =
                `/vintage-vault/week6b/shop.php?search=${encodeURIComponent(this.value)}`;
        }
    });
}
